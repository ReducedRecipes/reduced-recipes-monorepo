...
// Single handler for all sitemap files — Hono params don't handle dots well
app.use('/sitemap*', async (c, next) => {
  const path = new URL(c.req.url).pathname;
  if (!path.endsWith('.xml')) return next();

  let kvKey: string;
  if (path === '/sitemap.xml') {
    kvKey = 'sitemap:index';
  } else {
    const name = path.replace('/sitemap-', '').replace('.xml', '');
    kvKey = name === 'static' ? 'sitemap:static' : `sitemap:chunk:${name}`;
  }
